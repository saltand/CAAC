...
import { fetchCatImage, normalizeDimension, isClient } from '@caac/shared';
...
// Access runtime config for default API key
const { $config } = useNuxtApp();
const runtimeConfig = useRuntimeConfig();

// Use provided API key or fall back to runtime config
const effectiveApiKey = computed(() => 
  props.apiKey || runtimeConfig.public?.catImageApiKey
);
...
const change = async () => {
  ...
  try {
    const result = await fetchCatImage(effectiveApiKey.value);
...
// Initialize on mount (SSR-safe with Nuxt-specific checks)
onMounted(() => {
